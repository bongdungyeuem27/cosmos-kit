...
export class Web3AuthSigner {
    ...
    async getAccounts() {
        ...
        await sendAndListenOnce(this.#worker, {
            ...
            if (data.type === 'accounts' && data.payload.id === id) {
                // Verify signature.
                await eccrypto.verify(this.#workerPublicKey, hashObject(data.payload), Buffer.from(data.signature));
    ...
    async signDirect(signerAddress, signDoc) {
        ...
        message.signature = await eccrypto.sign(this.#clientPrivateKey, hashObject(message.payload));
        let response;
