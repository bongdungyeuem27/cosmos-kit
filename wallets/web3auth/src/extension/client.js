...
export class Web3AuthClient {
    ...
    async ensureSetup() {
        ...
        if ((this.#options?.loginProvider === LOGIN_PROVIDER.EMAIL_PASSWORDLESS ||
            this.#options?.loginProvider === LOGIN_PROVIDER.SMS_PASSWORDLESS) &&
            this.#loginHint === undefined) {
            throw new Error('Login hint is required for email/sms passwordless login');
        }
        if (!this.#options) {
        ...
        const { client, provider } = await connectClientAndProvider(this.env.device === 'mobile', {
            ...this.#options,
        ...
        const encryptedPrivateKey = await eccrypto.encrypt(workerPublicKey, Buffer.from(privateKeyHex, 'hex'));
        await sendAndListenOnce(worker, {
            type: 'init_2',
            payload: {
                encryptedPrivateKey,
    ...
    async disconnect() {
        ...
        try {
            const { client } = await connectClientAndProvider(this.env.device === 'mobile', this.#options, { dontAttemptLogin: true });
            if (client.connected) {
                await client.logout({
                    cleanup: true,
                });
