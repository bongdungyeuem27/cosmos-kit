...
import { COSMIFRAME_KEYSTORECHANGE_EVENT, COSMIFRAME_WALLET_ID, } from './cosmiframe/constants';
...
export class WalletManager extends StateBase {
    ...
    init(chains, assetLists, wallets, walletConnectOptions, signerOptions, endpointOptions) {
        ...
        this.chainRecords = chains.map((chain) => {
            ...
            const converted = convertChain(chain, assetLists, signerOptions, endpointOptions?.endpoints?.[chainName], this.isLazy, this.logger);
            return converted;
        });
    ...
    addChains = (chains, assetLists, signerOptions, endpoints) => {
        const newChainRecords = chains.map((chain) => {
            const chainName = typeof chain === 'string' ? chain : chain.chain_name;
            return convertChain(chain, assetLists, signerOptions, endpoints?.[chainName], this.isLazy, this.logger);
        });
